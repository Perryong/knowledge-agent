...
## Hybrid Raw-First Layer

COG is the main framework. The hybrid layer adds strict source preservation, PARA organization inside `05-knowledge/`, and schema compilation. See `docs/HYBRID-SECOND-BRAIN.md`.

**Raw-first rule:** every external source must be captured under `raw/<category>/YYYY-MM-DD-<slug>/` with `metadata.md` before summaries, analysis, PRDs, stories, or durable knowledge notes are created.

**Hybrid commands:**
- `/raw-capture` - preserve source material before processing
- `/para-classify` - classify durable knowledge into `05-knowledge/projects|areas|resources|archives`
- `/schema-compile` - extract concepts and relationships into `05-knowledge/schema/`
- `/raw-inbox-review` - find raw captures that still need processing
