...
## Raw-First Research Rule

Before synthesizing findings, preserve important external sources under `raw/research/YYYY-MM-DD-[topic-slug]/` or `raw/articles/YYYY-MM-DD-[source-slug]/` with `metadata.md`. The final research output must include `raw_sources` links for sources that shaped the analysis.
